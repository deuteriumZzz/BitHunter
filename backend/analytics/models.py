...
class AnalyticsData(models.Model):
    ...
    news_sentiment = models.FloatField(
        default=0.0, help_text="Средний sentiment новостей за период"
    )
    related_news = models.ManyToManyField(
        News, blank=True, help_text="Связанные новости"
    )
...
class Prediction(models.Model):
    ...
    timestamp = models.DateTimeField(
        auto_now_add=True, verbose_name="Время предсказания"
    )
    predicted_price = models.FloatField(verbose_name="Предсказанная цена")
    action = models.IntegerField(choices=ACTION_CHOICES, verbose_name="Рекомендация")
    profit_loss = models.FloatField(default=0.0, verbose_name="Прибыль/убыток")
...
class Trade(models.Model):
    ...
    sell_price = models.FloatField(null=True, blank=True, verbose_name="Цена продажи")
    ...
    sell_date = models.DateTimeField(null=True, blank=True, verbose_name="Дата продажи")
